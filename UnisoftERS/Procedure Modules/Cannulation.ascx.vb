...
Public Class Cannulation
    ...
    Protected Sub Load_Data()
        ...
        If trainEE_Exist Then
            tdTrainEE.Visible = True
            lblTrainERName.Visible = True
            lblTrainERName.Text = CStr(drEndoscopist("TrainerEndoscopist"))

            lblTrainEEName.Visible = True
            lblTrainEEName.Text = CStr(drEndoscopist("TraineeEndoscopist"))
        ...
        Else
            tdTrainEE.Visible = False
            lblTrainERName.Visible = False
            lblTrainEEName.Visible = False
        ...
        If VisRecord IsNot Nothing Then
            ...
            Select Case GetNumber(VisRecord.SphincterotomyAttempts)
                Case 1
                    optSphincterotomy1_ER.Checked = True
                Case 2
                    optSphincterotomy2_ER.Checked = True
                Case 3
                    optSphincterotomy3_ER.Checked = True
                Case 4
                    optSphincterotomy4_ER.Checked = True
                    ER_SphincterotomyOtherDiv.Visible = True
                    ER_SphincterotomyOtherTextBox.Text = VisRecord.SphincterotomyAttemptsOtherText
            End Select
    ...
    Private Sub saveRecord()
        Try
            ...
            'DB.SaveVisualisation(VisRecord, NEW_RECORD, True)

            ScriptManager.RegisterStartupScript(Me.Page, GetType(Page), "build-summary", "setRehideSummary();", True)
        Catch ex As Exception
            Dim errorLogRef As String
            errorLogRef = LogManager.LogManagerInstance.LogError("There was an error autosaving cannulation data", ex)

            Utilities.SetErrorNotificationStyle(RadNotification1, errorLogRef, "There is a problem saving your cannulation data.")
    ...
    Sub Fill_Record_Object()
        Try
            ...
            Using db As New ERS.Data.GastroDbEntities
                ...
                VisRecord = db.ERS_Visualisation.Where(Function(x) x.ProcedureID = PROCEDURE_ID).FirstOrDefault

                If VisRecord Is Nothing Then
                    NEW_RECORD = True
                    VisRecord = New ERS.Data.ERS_Visualisation
                    VisRecord.ProcedureID = PROCEDURE_ID
                Else
                    NEW_RECORD = False
                End If
                ...
                'VisRecord.ProcedureID = CInt(Session(Constants.SESSION_PROCEDURE_ID)) '### Preparing for INSERT or UPDATE -> in any case feed the ProcedureId.. remember this is a new Instance Object- empty! Feed it...
                ...
                If optSphincterotomy1_ER.Checked Then
                    VisRecord.SphincterotomyAttempts = 1
                    VisRecord.SphincterotomyAttemptsOtherText = Nothing
                ElseIf optSphincterotomy2_ER.Checked Then
                    VisRecord.SphincterotomyAttempts = 2
                    VisRecord.SphincterotomyAttemptsOtherText = Nothing
                ElseIf optSphincterotomy3_ER.Checked Then
                    VisRecord.SphincterotomyAttempts = 3
                    VisRecord.SphincterotomyAttemptsOtherText = Nothing
                ElseIf optSphincterotomy4_ER.Checked Then
                    VisRecord.SphincterotomyAttempts = 4
                    If Trim(ER_SphincterotomyOtherTextBox.Text) <> "" Then VisRecord.SphincterotomyAttemptsOtherText = ER_SphincterotomyOtherTextBox.Text Else VisRecord.SphincterotomyAttemptsOtherText = Nothing
                Else
                    VisRecord.SphincterotomyAttempts = Nothing
                    VisRecord.SphincterotomyAttemptsOtherText = Nothing
                End If
                ...
                If NEW_RECORD Then
                    db.ERS_Visualisation.Add(VisRecord)
                    NEW_RECORD = False
                End If
                db.SaveChanges()
                Dim da As DataAccess = New DataAccess()
                da.Visualisation_DuplicateCheck(PROCEDURE_ID)
            End Using
        Catch ex As Exception
            Dim errorLogRef As String
            errorLogRef = LogManager.LogManagerInstance.LogError("ProcedureModules\Cannulation.aspx.vb, at: Fill_Record_Object()", ex)
