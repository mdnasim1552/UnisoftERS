...
Public Class Stenosis
    ...
    Private Sub PopulateData(drSc As DataRow)
        If Not IsDBNull(drSc("Stenosis")) AndAlso CInt(drSc("Stenosis")) > 0 Then
            StenosisRBL.SelectedValue = CInt(drSc("Stenosis"))
    ...
    Protected Sub SaveRecord(saveAndClose As Boolean)
        Try
            ...
            AbnormalitiesDataAdapter.SaveBRTAbnosData(
                ...
                Nothing,
                Nothing,
                Utilities.GetRadioValue(StenosisRBL),
                Nothing,
                Nothing,
                Nothing,
                Nothing,
            ...
            Using db As New ERS.Data.GastroDbEntities
                ...
                ersRecord = db.ERS_RecordCount.Where(Function(ers) ers.SiteId = siteId And ers.Identifier = "Stenosis").FirstOrDefault()
        ...
        Catch ex As Exception
            ...
            errorLogRef = LogManager.LogManagerInstance.LogError("Error occured while saving Bronchoscopy/EBUS Abnormalities - Stenosis.", ex)
