...
Public Class CarinalAbnormality
    ...
    Private Sub PopulateData(drSc As DataRow)
        If Not IsDBNull(drSc("Carinal")) AndAlso CInt(drSc("Carinal")) > 0 Then
            CarinalRBL.SelectedValue = CInt(drSc("Carinal"))
    ...
    Protected Sub SaveRecord(saveAndClose As Boolean)
        Try
            ...
            AbnormalitiesDataAdapter.SaveBRTAbnosData(
                ...
                Utilities.GetRadioValue(CarinalRBL),
                Nothing,
                Nothing,
                Nothing,
                Nothing,
            ...
            Using db As New ERS.Data.GastroDbEntities
                ...
                ersRecord = db.ERS_RecordCount.Where(Function(ers) ers.SiteId = siteId And ers.Identifier = "Carinal Abnormality").FirstOrDefault()
                ...
                If ersRecord Is Nothing Then
                    ...
                    ersRecord.Identifier = "Carinal Abnormality"
            ...
            'ScriptManager.RegisterStartupScript(Me.Page, GetType(Page), "ScriptRefreshParent", "refreshParentEBUS('" & siteId & "');", True) 'Added by rony tfs-4166
        Catch ex As Exception
            Dim errorLogRef As String
            errorLogRef = LogManager.LogManagerInstance.LogError("Error occured while saving Bronchoscopy/EBUS Abnormalities - Bleeding.", ex)
