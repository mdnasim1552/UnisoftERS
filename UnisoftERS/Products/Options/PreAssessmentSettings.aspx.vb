...
Partial Class Products_Options_PreAssessmentSettings
    ...
    Private Function GetSections() As DataTable
        Using connection As New SqlConnection(DataAccess.ConnectionStr)
            Dim cmd As New SqlCommand("SELECT SectionId, SectionName FROM ERS_PreAssessmentSections where Suppressed = @suppressed", connection)
    ...
    Protected Sub AddNewItemSaveRadButton_Click(sender As Object, e As EventArgs) Handles AddNewItemSaveRadButton.Click
        ...
        If AddNewItemSaveRadButton.Text = "Update" Then
            da.UpdateSectionList(CInt(hiddenItemId.Value), AddNewItemRadTextBox.Text, sortOrder, Nothing)
            SectionRadGrid.Rebind()
        ElseIf AddNewItemRadTextBox.Text <> "" Then
            newItemId = da.InsertSectionsItem(AddNewItemRadTextBox.Text, sortOrder)
    ...
    Protected Sub ListsRadGrid_ItemCommand(sender As Object, e As GridCommandEventArgs) Handles SectionRadGrid.ItemCommand
        If e.CommandName = "SuppressItem" Then
            ...
            If CType(e.CommandSource, LinkButton).Text.ToLower = "unsuppress" Then
                bSuppress = False
            End If
            DataAdapter.SuppressSectionList(CInt(DirectCast(e.Item, GridDataItem).GetDataKeyValue("SectionId")), bSuppress)
            SectionRadGrid.Rebind()
        End If
    End Sub
    ...
    Protected Sub SectionRadGrid_ItemCreated(ByVal sender As Object, ByVal e As GridItemEventArgs) Handles SectionRadGrid.ItemCreated
        If TypeOf e.Item Is GridDataItem Then
            ...
            Else
                sSortOrder = ""
            End If
            EditLinkButton.Attributes("onclick") = String.Format("return openAddItemWindow('{0}','{1}','{2}');", e.Item.OwnerTableView.DataKeyValues(e.Item.ItemIndex)("SectionId"), sItemName, sSortOrder)
        End If
    ...
    Protected Sub QuestionsRadGrid_ItemCommand(sender As Object, e As GridCommandEventArgs) Handles QuestionRadGrid.ItemCommand
        If e.CommandName = "QuestionSuppressItem" Then
            ...
            If CType(e.CommandSource, LinkButton).Text.ToLower = "unsuppress" Then
                bSuppress = False
            End If
            DataAdapter.SuppressQuestionList(CInt(DirectCast(e.Item, GridDataItem).GetDataKeyValue("QuestionId")), bSuppress, Nothing, Nothing, Nothing, Nothing)
            QuestionRadGrid.Rebind()
        End If
    End Sub
    Protected Sub AddNewQuestionSaveRadButton_Click(sender As Object, e As EventArgs) Handles AddNewQuestionSaveRadButton.Click
        ...
        If AddNewQuestionSaveRadButton.Text = "Update" Then
            da.UpdateQuestionList(CInt(HiddenItemId1.Value), selectedSectionId, AddNewQuestionRadTextBox.Text, chkOptionalRadComboBox.Checked, chkFreeTextRadComboBox.Checked, chkYesNoRadComboBox.Checked, DropdownOption.Checked, DropdownOptionTextBox.Text, Nothing, sortOrder)
            QuestionRadGrid.Rebind()
        ElseIf AddNewQuestionSaveRadButton.Text <> "" Then
            newItemId = da.InsertQuestionItem(selectedSectionId, AddNewQuestionRadTextBox.Text, chkOptionalRadComboBox.Checked, chkFreeTextRadComboBox.Checked, chkYesNoRadComboBox.Checked, DropdownOption.Checked, DropdownOptionTextBox.Text, False, sortOrder)
            AddNewItemRadTextBox.Text = ""
            QuestionRadGrid.Rebind()
        End If
    ...
    Protected Sub QuestionRadGrid_ItemCreated(ByVal sender As Object, ByVal e As GridItemEventArgs) Handles QuestionRadGrid.ItemCreated
        If TypeOf e.Item Is GridDataItem Then
            ...
            Else
                sSectionId = ""
            End If
            If (DataBinder.Eval(e.Item.DataItem, "Optional") IsNot Nothing AndAlso Not [String].IsNullOrEmpty(DataBinder.Eval(e.Item.DataItem, "Optional").ToString())) Then
                sOptional = e.Item.DataItem("Optional")
            Else
                sOptional = False
            End If
